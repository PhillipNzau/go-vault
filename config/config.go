...
func ensureIndexes(cfg *Config) error {
	...
	// subscriptions: user_id and renewal_date
	subs := db.Collection("subscriptions")
	_, err = subs.Indexes().CreateMany(context.Background(), []mongo.IndexModel{{Keys: bsonD{{Key: "user_id", Value: 1}}}, {Keys: bsonD{{Key: "renewal_date", Value: 1}}}})
	return err
}
